...
atomic<bool>exit_micproc{false};
	...
	*pipc->pusb >> pipc->frame;
	...
	if(pipc->wq.size()){
		...
		case(FR_INIT):{
			...
			pipc->pvideo_output->open(fn.c_str(),VideoWriter::fourcc('m', 'p', '4', 'v'),1.0,pipc->frame.size(),true);
		}
		case(FR_RUN):{
...
static void on_playback_process(void *userdata){
	appData *app = static_cast<appData *>(userdata);
	pipc->mx_aoutq.lock();
...
static void on_capture_process(void *userdata){
	...
	struct pw_buffer *b;
	struct spa_data *d;
	if((b = pw_stream_dequeue_buffer(app->capture_stream)) == NULL)return;
	d = &b->buffer->datas[0];
	if(d->data != NULL){
...
void *tcpproc(void *){
	...
	string port = to_string(TCP_PORT);
 	while(!isValidIP(sip)){
       		sip = "sip";
        	access_dbase(sip,DBSTRING);
#ifdef DEBUG
		cout << "TCPPROC " << sip.c_str() << " " << sip.length() << " " << port << endl;
#endif
		sleep(1);
	}

	boost::asio::io_context io_context;
	AsyncTcpClient client(io_context,sip,port);
	pipc->ptcp = &client;
	syslog(LOG_INFO,"started tcpproc");
       	pipc->tp_state = true;
	try{
		io_context.run();
	}catch(std::exception& e){
        	syslog(LOG_INFO,"tcpproc exception");
       		pipc->tp_state = false;
	}
       	pipc->tp_state = false;
        syslog(LOG_INFO,"tcpproc stopped");
	return NULL;
}
...
void *netproc(void *){
	...
	vector<unsigned char> bytes;
        ...
        while(!exit_netproc){
                ...
                pw->receive();
		if(!pipc->tp_state){
			pthread_create(&th_tcpproc_id,NULL,tcpproc,nullptr);
        		while(!pipc->tp_state);
        		syslog(LOG_INFO,"netproc tcpproc started");
		}

                if(pm->rxfifo.size()){
                        up = pm->rxfifo[0];
                        string msg((char *)&up.data,up.len);
                        stringstream ss(msg);
                        vector<string> word;
                        string w;
		...
		if(pw->rxfifo.size()){
			...
			if(whr == hr){
				...
				if(wfl >= whi)mrun = false;
                        ...
                        up.len = m_msg.length();
                        memcpy(up.data,m_msg.c_str(),up.len);
                        pm->txfifo.push_back(up);
                        pm->sender();

                        up.len = w_msg.length();
                        memcpy(up.data,w_msg.c_str(),up.len);
                        pw->txfifo.push_back(up);
                        pw->sender();
#ifdef DEBUG
		...
		if(pipc->ptcp->hq.size()){
			pipc->ptcp->mx_hq.lock();
			string msg = bytes_to_hex(pipc->ptcp->hq[0].data,pipc->ptcp->hq[0].len);
			pipc->ptcp->hq.erase(pipc->ptcp->hq.begin());
			pipc->ptcp->mx_hq.unlock();
			msg += "  ";
			switch(msg.length()){
				case(282):{
	...
	if(pipc->tp_state)pthread_join(th_tcpproc_id,NULL);
        if(pm)delete pm;
        if(pw)delete pw;
...
void *micproc(void *){
        ...
        CPU_SET(1,&cpuset);
        pthread_t current_thread = pthread_self();
        if(pthread_setaffinity_np(current_thread, sizeof(cpu_set_t),&cpuset))syslog(LOG_INFO,"micproc thread afinity to CPU 1 failed");
        pthread_setschedprio(pthread_self(),255);
	pw_init(nullptr,nullptr);
	appData app;
	memset((void *)&app,0,sizeof(app));

	app.thread_loop = pw_thread_loop_new("io_loop", nullptr);
	if(!app.thread_loop){
        	syslog(LOG_INFO,"micproc failed to create thread loop");
		sighandler(200);
	}

	const struct pw_stream_events capture_events = {
		PW_VERSION_STREAM_EVENTS,
		.process = on_capture_process
	};
    	app.capture_stream = pw_stream_new_simple(pw_thread_loop_get_loop(app.thread_loop),"cpp-capture",pw_properties_new(PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Capture", NULL),&capture_events,(void *)&app);
	if(!app.capture_stream){
        	syslog(LOG_INFO,"micproc failed to create stream");
		pw_thread_loop_destroy(app.thread_loop);
		sighandler(201);
	}
	const struct spa_pod *capture_params[1];
	uint8_t capture_buffer[1024];
	struct spa_pod_builder capture_b = SPA_POD_BUILDER_INIT(capture_buffer, sizeof(capture_buffer));
	struct spa_audio_info_raw capture_audio_info = SPA_AUDIO_INFO_RAW_INIT(.format = SPA_AUDIO_FORMAT_S16_LE,.rate = 8000,.channels = 1);
	capture_params[0] = spa_format_audio_raw_build(&capture_b, SPA_PARAM_EnumFormat,&capture_audio_info);
	pw_stream_connect(app.capture_stream, PW_DIRECTION_INPUT, PW_ID_ANY, PW_STREAM_FLAG_AUTOCONNECT, capture_params, 1);

	const struct pw_stream_events playback_events = {
		PW_VERSION_STREAM_EVENTS,
		.process = on_playback_process
	};
	app.playback_stream = pw_stream_new_simple(pw_thread_loop_get_loop(app.thread_loop),"cpp-playback",pw_properties_new(PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Playback", NULL),&playback_events,(void *)&app);
	...
	if(pw_thread_loop_start(app.thread_loop) < 0){
	       	syslog(LOG_INFO,"micproc failed to start thread loop");
		pw_thread_loop_destroy(app.thread_loop);
		sighandler(203);
	}
        string cmd = "vth";
        access_dbase(cmd,DBINT);
        unsigned short vth = 0;
	if(isuint(cmd))vth = stoi(cmd);
	fft afft(vth);
	...
	bool prev_ac = false;
        ...
        syslog(LOG_INFO,"started micproc");
        while(!exit_micproc){
                pipc->mi_state = true;
		if(pipc->ac != prev_ac){
	        	vector <bool> sig;
			...
			if(pipc->ac){
				ac_cmd = on;
				syslog(LOG_INFO,"micproc AC on");
			}else{
			       	ac_cmd = off;
				syslog(LOG_INFO,"micproc AC off");
			}
		        vector <bool> bin;
	...
	pw_stream_destroy(app.capture_stream);
	pw_stream_destroy(app.playback_stream);
	pw_thread_loop_destroy(app.thread_loop);
	pw_deinit();
        syslog(LOG_INFO,"micproc stopped");
        return NULL;
}
...
int main(void){
	ipc ip;
        ...
        close(STDERR_FILENO);
#endif
	pipc->frame = Mat(FRAME_H,FRAME_W,CV_8UC3,Scalar(0,0,0));
...
#endif
	timer_delete(pipc->timerid);
        pthread_join(th_micproc_id,NULL);
