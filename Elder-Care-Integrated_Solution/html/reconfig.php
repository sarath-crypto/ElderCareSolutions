<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$wip = htmlspecialchars($_POST['wip']);
		$bip = htmlspecialchars($_POST['bip']);
		$sn = htmlspecialchars($_POST['sn']);
		$mkey = htmlspecialchars($_POST['mkey']);
		$wkey = htmlspecialchars($_POST['wkey']);
		$bkey = htmlspecialchars($_POST['bkey']);
		$whr = htmlspecialchars($_POST['whr']);
		$wlo = htmlspecialchars($_POST['wlo']);
		$whi = htmlspecialchars($_POST['whi']);
        ...
        if($_SESSION['auth']){
			...
			if($wip)$sql = $sql."wip='".$wip."',";	
			if($bip)$sql = $sql."bip='".$bip."',";	
			if($sn)$sql = $sql."sn='".$sn."',";	
			if($mkey)$sql = $sql."mkey='".$mkey."',";	
			if($wkey)$sql = $sql."wkey='".$wkey."',";	
			if($bkey)$sql = $sql."bkey='".$bkey."',";	
			if($whr)$sql = $sql."whr='".$whr."',";	
			if($wlo)$sql = $sql."wlo='".$wlo."',";	
			if($whi)$sql = $sql."whi='".$whi."',";	
