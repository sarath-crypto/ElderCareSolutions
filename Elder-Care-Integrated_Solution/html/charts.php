...
function drawLineGraph_sensor($cachefilename,$ptemp,$phumd,$pnoise,$pwl,$px){
	require_once ('jpgraph/jpgraph.php');
	require_once ('jpgraph/jpgraph_line.php');
	...
	if ($valid){
		...
		$p1 = new LinePlot($ptemp);
		$graph->Add($p1);
		$p1->SetWeight(3);
		$p1->SetColor("#f44336");
		$val = end($ptemp);
		$p1->SetLegend('Temperature ['.$val.'c]');
		...
		$p2->SetColor("#27cff5");
		$val = end($pwl);
		$p2->SetLegend('Water Level ['.$val.']');
		
		$p3 = new LinePlot($pnoise);
		$graph->Add($p3);
		$p3->SetWeight(3);
		$p3->SetColor("#888E96");
		$val = end($pnoise);
	...
	if ($conn->connect_error) {
		...
		$min_noise = min($noise);
		$max_noise = max($noise);
		$scale = $max_noise-$min_noise;
		if($scale > 200){
			$scale = 200/$scale;
		}	
		foreach ($noise as $val){
			$mnoise[] = ($val-$min_noise)*$scale;
		}
		
		$f_sensor = 'graph/sensor.png';
		$graph = drawLineGraph_sensor($f_sensor,$temp,$humd,$noise,$wl,$ts);
		echo '<tr><td>';
