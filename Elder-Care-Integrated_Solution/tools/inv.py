...
while chunks<2:
 ...
 while (not ok):
  try:
   ...
   sys.exit(1) #die
 frame_hex = binascii.hexlify(data);
 print("Receive ");
 print(frame_hex);
 ...
 while a<=i:
  p1=56+(a*4)
  p2=60+(a*4)
  #convert binary to hexstring and add two spaces at end
  ...
  with open("DeyeJson.txt") as txtfile:
     ...
     for register in item["registers"]:
      if register==hexpos and chunks!=-1:
       if title.find("Temperature")!=-1: 
        ...
        print(chunks," ",int(hexpos,0)," ",hexpos+"-"+title+":"+str(response*ratio)+unit,p1,p2)
  a+=1
 pini=150
 pfin=195
 chunks+=1  
