...
void *mdproc(void *){
	...
	syslog(LOG_INFO,"started mdproc");
		...
		if(pipc->vq.size()){
			...
			pipc->mx_vlvl.unlock();
			putText(frame,header.c_str(),cv::Point(0,475),FONT_HERSHEY_SIMPLEX,0.7,Scalar(0,0,250),1);
			
			string val = "BAT["+to_string(pipc->bl)+"%]";
			putText(frame,val,cv::Point(0,390),FONT_HERSHEY_SIMPLEX,0.7,Scalar(0,0,250),1);
			unsigned short v = ((float)pipc->sl/(float)3240)*100;
			val = "SPWR["+to_string(v)+"%]";
			putText(frame,val,cv::Point(0,420),FONT_HERSHEY_SIMPLEX,0.7,Scalar(0,0,250),1);
			val = "GRID_PWR";
			if(pipc->grid)val += "[ON]";
			else val += "[OFF]";
