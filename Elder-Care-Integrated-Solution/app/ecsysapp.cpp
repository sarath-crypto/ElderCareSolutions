...
void *dbproc(void *){
	...
	size_t pos = ts.find(':');
	unsigned char hr = stoi(ts.substr(0,pos)); 

	if(pipc->temp.size() < (unsigned int)((hr-1)*26))for(int i = 0;i < (hr-1)*26;i++)pipc->temp.push_back(pipc->rt);

        syslog(LOG_INFO,"started dbproc");
        while(!exit_dbproc){
		gettimestamp(ts,false);
		pos = ts.find(':');
		hr = stoi(ts.substr(0,pos)); 
		...
		if(!pipc->pcam->get_frame(frame)){
			...
			vector <unsigned short> tp = pipc->temp;
			unsigned short min = *min_element(tp.begin(),tp.end());
			transform(tp.begin(),tp.end(),tp.begin(),[min](int val) { return val - min; });
			unsigned short max = *max_element(tp.begin(),tp.end());
			double scale  = 0;
			if(max)scale  = (double)PLOT_MAX/(double)max;
			for(unsigned int i = 0;i < tp.size();i++)line(frame,Point(i+8,480),Point(i+8,480-(tp[i]*scale)),Scalar(255,220,0),1,LINE_AA);
			tp.clear();
			...
			header += "]";	
			putText(frame,header.c_str(),Point(0,24),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);

			header = "AC:";
			if(pipc->ac)header += "ON ";
			else header += "OFF ";
			header += to_string(pipc->rt/100)+"."+to_string(pipc->rt%100)+"c";

			if(pipc->voice)header += " VoiceLevel:"+to_string(pipc->voice_level);
			putText(frame,header.c_str(),Point(0,475),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);

			unsigned char v = ((float)pipc->wl/(float)pipc->whi)*100;
			if(v  > 100)v = 100;
			string val = "WL["+to_string(v)+"%]";
			putText(frame,val,Point(0,360),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);
			val = "BAT["+to_string(pipc->bl)+"%]";
			putText(frame,val,Point(0,390),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);
			v = ((float)pipc->sl/(float)3240)*100;
			val = "SPWR["+to_string(v)+"%]";
			putText(frame,val,Point(0,420),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);
			val = "GRID_PWR";
			if(pipc->grid)val += "[ON]";
			else val += "[OFF]";
			putText(frame,val,Point(0,450),FONT_HERSHEY_TRIPLEX,0.7,Scalar(0,0,250),1);
...
void *audioproc(void *){
	...
	cmd = "aco";
	access_dbase(cmd,DBINT);
	unsigned short aco = stoi(cmd)*100;
...
int main(void){
	...
	int th = 0;
	unsigned long int tacc = 0;
	unsigned char prev_hr = 0;
	while(!exit_main){
		ifstream ifs("/sys/bus/iio/devices/iio:device0/in_temp_input");
		if(ifs.is_open()){
			string line;
			getline(ifs,line);
			if(line.length() > 4){
				pipc->rt = stoi(line)/10;
				tacc += pipc->rt;
				th++;
			}
			ifs.close();
